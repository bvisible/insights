...
Committed build artifacts, rewritten by every build (commit-the-build): a marker written into
them would be wiped by the next build, so the decision is marked at its source (`.gitignore`,
`frontend/index.html`, `package.json`) and the check skips these paths.

| Artifact | Generated by |
|---|---|
| `insights/www/insights.html` | `yarn copy-html-entry` from the vite output; its asset hashes change at every build |
| `insights/www/insights_v2.html` | `yarn copy-html-entry2`, the legacy v2 entry, same |
