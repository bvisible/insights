...
(function () {
	...
	//// Neoffice — the titles are translated when the banner renders: upstream printed
	//// them in English, and __() only knows the boot messages once the desk has
	//// started, after this bundle loaded.
	const ACCOUNTING = {
		title: () => __("Receivables, Payables & Cash"),
		template: "accounting",
	};
	const DASHBOARDS = {
		Selling: { title: () => __("Sales Performance"), template: "sales" },
		Buying: { title: () => __("Purchasing Overview"), template: "purchasing" },
		Stock: { title: () => __("Stock & Inventory"), template: "stock" },
